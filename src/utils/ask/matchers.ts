...
export function matchAiPrompt(input: NormalizedAskInput): AskResult | null {
    const text = input.cleaned;
    ...
    if (questionStarters.some((starter) => text === starter || text.startsWith(`${starter} `))) {
        return useAi();
    }
    ...
    if (currentInfoHints.some((hint) => text.includes(hint))) {
        return useAi();
    }
    ...
    if (aiTaskHints.some((hint) => text.includes(hint))) {
        return useAi();
    }
